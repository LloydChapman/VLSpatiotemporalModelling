...
actvK=((data.KA<=origin&data.KARX>origin)|(data.KAYR==startyr-1&isnan(data.KARX)&~isinf(simdata.tR)))&isnan(data.MIG_IN); % (potentially) active KA at start of study
prevK=(data.KAYR<startyr&~actvK); % KA before the start of the study
IpreEXTIM=false(n,1); %(K02_10&data.EXTMIG_IN==1&data.KA<=data.MIG_IN&data.KA>origin+maxIP); % KA onset before or at migration in
EXTIMsoonI=false(n,1); %(K02_10&data.EXTMIG_IN==1&data.KA>=data.MIG_IN&data.KA-data.MIG_IN<=maxIP_IM&data.KA>origin+maxIP); % KA onset within 6 months of migration in
...
% Zero out infectiousness for PKDL cases with onset after tmax
hv(~P)=0;
...
if nAmoves==0
    ...
    tRA=NaN(n,1);
    tRA(setdiff(Asx,P))=simdata.tR(setdiff(Asx,P));
    tRA(PA)=simdata.tDor(PA);
    tA(actvA)=0;
    tRA(actvA)=simdata.tR(actvA);
    tA(prevA)=0;
    tRA(prevA)=0;
else
...
% for i=1:niters
% pickA(:,i)=SusA(randperm(nSusA,nAmoves));
% end
